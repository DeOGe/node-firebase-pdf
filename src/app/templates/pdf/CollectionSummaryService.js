...
class CollectionSummaryService {
    ...
    pageTwo = (doc, data) => new Promise((resolve) => {
        ...
        doc.text('Page 2', 30, 50, {
            align: 'left',
            width: 550
        });

        doc.text('P a y m e n t   C o l l e c t i o n   S e r v i c e', 30, 80, {
            align: 'left',
            width: 550,
        });

        doc.text('D A I L Y  C O L L E C T I O N  A N D  D E P O S I T  R E P O R T', 30, 100, {
            align: 'left',
            width: 550,
        });

        doc.text('BayadCenter Code :', 30, 120, {
            align: 'left',
            width: 550, 
        });

        doc.text('TPA ID: 0000', 30, 140, {
            align: 'left',
            width: 550,
        });

        doc.text('Merchant  -  Social Security System', 30, 170, {
            align: 'left',
            width: 550,
        });
        ...
        doc.text('Cash Amt/Cnt', 200, 220, {
            align: 'left',
            width: 200,
        });
        doc.text('Check Amt/Cnt', 350, 220, {
            align: 'left',
            width: 200,
        });
        doc.text('Total Amt', 500, 220, {
            align: 'left',
            width: 300,
        });

        doc.text('COLLECTIONS', 30, 250, {
            align: 'left',
            width: 550,
        });

        doc.text(`${total.cash.transactions}`, 230, 250, {
            align: 'left',
            width: 200,
        });

        doc.text(`${total.check.transactions}`, 390, 250, {
            align: 'left',
            width: 200,
        });

        doc.text(`${total.all.transactions}`, 500, 250, {
            align: 'left',
            width: 200,
        });
        
        doc.text('VALIDATED DEPOSITS', 30, 300, {
            align: 'left',
            width: 550,
        });

        doc.text('________________', 300, 380, {
            align: 'right',
            width: 250,
        });

        doc.text('Total Deposits', 200, 400, {
            align: 'center',
            width: 250,
        });

        doc.text(`${amountFormat(data.deposit)}`, 390, 400, {
            align: 'center',
            width: 250,
        });

        doc.text('Difference', 200, 430, {
            align: 'center',
            width: 250,
        });
        let difference = `${parseFloat(data.deposit.replace(/,/g, ''))}` - `${parseFloat(total.all.transactions.replace(/,/g, ''))}`;
        doc.text(`${amountFormat(difference)}`, 390, 430, {
            align: 'center',
            width: 250,
        });

        doc.text('________________', 300, 450, {
            align: 'right',
            width: 250,
        });

        doc.text('________________', 320, 470, {
            align: 'right',
            width: 250,
        });

        doc.text('Noted By:__________________', 300, 520, {
            align: 'right',
            width: 250,
        });

        doc.text('Supervisor / Manager', 300, 550, {
            align: 'right',
            width: 250,
        });

        doc.text('(sign over printed name)', 310, 570, {
            align: 'right',
            width: 250,
        });
        resolve();
